...
<body class="antialiased flex flex-col items-center justify-between relative min-h-screen bg-neutral-100">
  ...
  <main class="flex min-w-[1280px] w-[90%] gap-8">
    {{-- SECTION FILTER --}}
    <section class="bg-white rounded-md shadow-md my-8 w-[15rem] h-[15rem] flex flex-col items-center justify-center">
      <form action="/mart/kategori-produk/{{ request()->id_kategori }}" method="GET" class="flex flex-col gap-4 p-2 w-full ">
        @csrf
